function solve(input) {
    ...
    function compareHeroes(a, b) {
        let [aName, aInfo] = a;
        let [bName, bInfo] = b;
        let byHealthDecending = bInfo.hp - aInfo.hp;

        if (byHealthDecending === 0) {
            return aName.localeCompare(bName);
        }

        return byHealthDecending;
    }
    ...
    function castSpell(heroes, heroName, mpNeeded, spellName) {
        ...
        if (hero.mp >= mpNeeded) {
            hero.mp -= mpNeeded;
            return `${heroName} has sucessfully cast ${spellName} and now has ${hero.mp} MP.`;
        }
    ...
    function takeDamage(heroes, heroName, damage, attacker) {
        ...
        if (hero.hp > 0) {
            return `${heroName} was hit for ${damage} HP by ${attacker} and now has ${hero.hp}`;
        }
